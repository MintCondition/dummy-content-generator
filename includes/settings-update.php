...
function dcg_display_update_info() {
    ...
    $last_update_check = get_transient('dcg_last_update_check');

    // If transients are not set, check for updates
    if (false === $latest_version || false === $last_update_check) {
        ...
        if ($github_data) {
            ...
            set_transient('dcg_last_update_check', $last_update_check, DAY_IN_SECONDS);
        }
    }

    $latest_version = $latest_version ?: 'Unknown';
    $last_update_check = $last_update_check ?: 'Never';

    ?>
    <h2><?php esc_html_e('DCG Updates', 'text-domain'); ?></h2>
    <p><?php esc_html_e('Installed Version: ', 'text-domain'); ?><strong id="dcg_installed_version"><?php echo esc_html($installed_version); ?></strong></p>
